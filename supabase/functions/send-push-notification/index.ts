...
serve(async (req: Request) => {
    ...
    try {
        ...
        // Check if Strategy A succeeded: must have recipients > 0 (not just no errors)
        const strategyAWorked = oneSignalResponse.ok && oneSignalData?.id && oneSignalData?.recipients > 0

        // ── Strategy B: ALWAYS run to reach Despia/native devices ──
        // Native Despia subscriptions are NOT linked to external_id (they use the iOS/Android
        // OneSignal SDK, not the Web SDK), so Strategy A will return recipients=0 for Despia users.
        // Strategy B uses the player_id stored in device_tokens, which is always available.
        ...
        if (!tokensError && tokens && tokens.length > 0) {
            ...
            strategyBResult = { status: fallbackResponse.status, body: fallbackData }
        } else {
            console.log('[PUSH] Strategy B: no device tokens found, skipping')
        }

        if (!strategyAWorked && !strategyBResult?.body?.id) {
            console.error('[PUSH] Both strategies failed to deliver')
            return new Response(JSON.stringify({
                error: 'No delivery: both strategies failed or no tokens',
                strategy_a: { status: oneSignalResponse.status, body: oneSignalData },
                strategy_b: strategyBResult ?? 'no_tokens'
        ...
        return new Response(JSON.stringify({
            success: true,
            strategy_a: { delivered: strategyAWorked, recipients: oneSignalData?.recipients || 0, id: oneSignalData?.id },
            strategy_b: strategyBResult ? { delivered: !!strategyBResult.body?.id, recipients: strategyBResult.body?.recipients || 0 } : 'skipped_no_tokens'
        }), { headers: corsHeaders })
