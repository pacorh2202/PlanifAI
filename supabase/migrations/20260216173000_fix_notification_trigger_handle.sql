-- Ensure handle column exists (idempotent)
DO $$
BEGIN
...
-- Update the notification function to be robust against NULL handles
...
BEGIN
  ...
  SELECT 
    ...
    ce.start_time,
    ce.end_time,
    -- COALESCE ensures we never get NULL, forcing fallback to user_name or 'Alguien'
    COALESCE(NULLIF(p.handle, ''), p.user_name, 'Alguien') as displayed_name
  INTO 
    event_record
  ...
  INSERT INTO public.notifications (
    ...
    'event_shared',
    event_record.title,
    '@' || event_record.displayed_name || ' te ha invitado a esta tarea', 
    jsonb_build_object(
