...
export const VoiceSettingsScreen: React.FC<VoiceSettingsScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] dark:bg-black relative overflow-hidden transition-colors duration-500">
      ...
      <header className="px-6 pb-4 flex items-center justify-between sticky top-0 z-30" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
        ...
        <h1 className="text-sm font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em]">{t.voice_profile}</h1>
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar px-6 pt-4 pb-40 relative z-10 flex flex-col">
        <section className="flex flex-col items-center justify-center py-10">
            <div className="relative group w-full max-w-sm flex flex-col items-center">
                <div className="relative w-32 h-32 mb-10 group cursor-default">
                    ...
                    <div className="relative w-full h-full rounded-full bg-black dark:bg-white flex items-center justify-center shadow-2xl overflow-hidden border-4 border-white/10 dark:border-black/5">
                ...
                <div className="relative w-full bg-white/40 dark:bg-gray-900/40 backdrop-blur-3xl rounded-[3.5rem] p-8 border border-white/60 dark:border-gray-800/60 shadow-xl flex flex-col items-center">
                    <p className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.3em] mb-6">{t.voice_personalize}</p>
                    <div className="grid grid-cols-[56px_1fr_56px] items-center w-full">
                        ...
                        <div className="flex flex-col items-center justify-center overflow-hidden">
                            {isEditing ? (
                                <input ref={inputRef} type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} onBlur={handleSaveName} onKeyDown={(e) => e.key === 'Enter' && handleSaveName()} className="text-center text-4xl font-black text-gray-900 dark:text-white bg-transparent border-none focus:ring-0 p-0 tracking-tighter w-full placeholder:opacity-20" placeholder="..." />
                            ) : (
                                <h2 onClick={handleStartEditing} className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter cursor-text truncate max-w-full">{assistantName}</h2>
                            )}
                            <div className={`h-1 rounded-full mt-3 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isEditing ? 'bg-indigo-500 w-full opacity-100' : 'bg-gray-200 dark:bg-gray-800 w-10 opacity-40'}`}></div>
                        </div>
                        <div className="flex justify-end">
                            <button onClick={isEditing ? handleSaveName : handleStartEditing} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-lg active:scale-90 ${isEditing ? 'text-white scale-110' : 'bg-white dark:bg-gray-800 text-gray-400 border border-gray-100 dark:border-gray-700'}`} style={{ backgroundColor: isEditing ? accentColor : '' }}>
        ...
        <section className="mt-8">
            <div className="flex items-center gap-3 mb-6 ml-2">
                <div className="w-8 h-8 rounded-full bg-gray-100 dark:bg-gray-900 flex items-center justify-center text-gray-400"><Mic2 size={14} /></div>
                <h2 className="text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.3em]">{t.voice_personality}</h2>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {[
                    ...
                    return (
                        <button key={voice.id} onClick={() => setAssistantVoice(voice.id as any)} className={`group relative flex items-center gap-5 p-6 rounded-[2.5rem] border-2 transition-all duration-500 ${isSelected ? 'bg-white dark:bg-gray-900 border-transparent shadow-xl translate-y-[-2px]' : 'bg-white/40 dark:bg-gray-900/20 border-gray-100 dark:border-gray-800 opacity-60'}`} style={{ borderColor: isSelected ? accentColor : 'transparent' }}>
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${isSelected ? 'text-white' : 'bg-gray-50 dark:bg-gray-800 text-gray-400'}`} style={{ backgroundColor: isSelected ? accentColor : '' }}>{voice.icon}</div>
                            <div className="flex-1 text-left">
                                <h3 className={`font-black text-base tracking-tight ${isSelected ? 'text-gray-900 dark:text-white' : 'text-gray-500'}`}>{voice.label}</h3>
                                <p className="text-[11px] text-gray-400 dark:text-gray-500 font-medium mt-0.5">{voice.desc}</p>
                            </div>
        ...
        <div className="mt-auto pt-14">
            ...
            <p className="text-[9px] text-gray-400 dark:text-gray-600 font-black uppercase tracking-[0.2em] mt-10 text-center opacity-60 leading-relaxed">{t.voice_confirm_desc}</p>
        </div>
      </main>
    </div>
  );
