...
const getRelativeTime = (date: string, t: any): string => {
    ...
    if (diffMins < 1) return t.notif_now;
    if (diffMins < 60) return `${diffMins}m`;
    if (diffHours < 24) return `${diffHours}h`;
    if (diffDays === 1) return t.notif_yesterday;
            ...
            <div className="w-full sm:w-[450px] h-[92dvh] sm:h-[85vh] bg-white/95 dark:bg-gray-900/95 backdrop-blur-2xl rounded-t-[2.5rem] sm:rounded-[3rem] shadow-2xl overflow-hidden flex flex-col pointer-events-auto transform transition-all duration-300 ease-out pb-safe">
                ...
                <div className="px-8 pt-8 pb-6 flex items-center justify-between border-b border-gray-100/50 dark:border-gray-800/50 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight" style={{ letterSpacing: '-0.02em' }}>{t.notif_header}</h2>
                    </div>
                    <button
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-6 rounded-[2.5rem] border transition-all duration-200 ${isUnread
                                            ...
                                            : 'bg-gray-50/80 dark:bg-gray-800/50 border-gray-50 dark:border-gray-800 opacity-80'
                                            }`}
                                    >
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <h3 className="font-bold text-gray-900 dark:text-white text-[17px] leading-tight mb-1.5 tracking-tight capitalize">
                                                    {config.title}
                                                </h3>
                                                <p className="text-[14px] text-gray-500 dark:text-gray-400 leading-snug tracking-tight font-medium mb-2">
                                                    {notif.message}
                                                </p>

                                                {/* Detalle de Horario */}
                                                {eventTime && (
                                                    <div className="flex items-center gap-1.5 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wide bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-xl w-fit">
                                                ...
                                                {isUnread && notif.type === 'event_shared' && (() => {
                                                    ...
                                                    return (
                                                        <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 rounded-2xl flex items-start gap-3">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                ...
                                                                <p className="text-[13px] text-amber-600 dark:text-amber-500/80 font-medium leading-tight">
                                                                    {t.notif_conflict_desc.replace('{title}', conflict.title)}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    );
                                                ...
                                                {isUnread && notif.type === 'event_shared' && (() => {
                                                    ...
                                                    return (
                                                        <div className="flex gap-4 mt-6">
                                                            ...
                                                            >
                                                                {conflict ? t.notif_prioritize : t.notif_accept}
                                                            </button>
                                                            <button
