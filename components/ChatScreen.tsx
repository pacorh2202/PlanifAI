...
export const ChatScreen: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] dark:bg-black relative overflow-hidden transition-colors duration-300">
      <div className="absolute inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-indigo-100 dark:bg-indigo-900/20 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-30 animate-blob"></div>
        <div className="absolute top-[20%] right-[-10%] w-[400px] h-[400px] bg-rose-100 dark:bg-rose-900/20 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-30 animate-blob animation-delay-2000"></div>
        <div className="absolute bottom-[-10%] left-[20%] w-[500px] h-[500px] bg-purple-100 dark:bg-purple-900/20 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-30 animate-blob animation-delay-4000"></div>
      </div>

      {/* Simplified Navigation Top Bar */}
      <div className="px-6 pb-2 z-20 flex justify-center items-center relative" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
      ...
      <div className="pt-4 pb-20 px-4 flex justify-center z-20" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 8rem)' }}>
        ...
        >
          ...
          <div className={`relative w-24 h-24 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 overflow-hidden border-2 ${connected ? 'bg-black dark:bg-white border-white/40 shadow-indigo-500/20' : 'bg-black dark:bg-white border-gray-800 shadow-xl'}`}>
