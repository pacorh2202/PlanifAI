...
export const getEventColor = (event: CalendarEvent, template: PlannerTemplate): string => {
  // 1. Check for Category Match (Prioritize Template Theme)
  if (event.categoryLabel) {
    const catByLabel = template.categories.find(c => c.label === event.categoryLabel);
  ...
  // 2. Fallback to custom color if set (but only if no category matched, or maybe we want custom override?)
  // Task 18: Shared tasks should use USER template color.
  // If we return category.color above, we enforce consistency.
  // If the event has a TRULY custom color (not just friend's template color), we might lose it.
  // But assuming most events follow categories:
  if (event.color) return event.color;

  return template.categories[0].color;
};
