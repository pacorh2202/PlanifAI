...
export const PrivacyScreen: React.FC<PrivacyScreenProps> = ({ onBack }) => {
  ...
  const sections = [
    {
      ...
      title: t.privacy_q1,
      content: (
        <ul className="space-y-3 mt-3 text-sm text-gray-600 dark:text-gray-400">
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8FAFC] dark:bg-black overflow-y-auto no-scrollbar pb-10 transition-colors duration-300">
      <header className="px-6 pb-6 flex items-center justify-between sticky top-0 bg-[#F8FAFC]/80 dark:bg-black/80 backdrop-blur-md z-20" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">{t.privacy_title}</h1>
        <div className="w-10"></div>
      </header>

      <main className="px-6 space-y-4 pt-2">
        {sections.map((section, idx) => (
          <div key={idx} className="bg-white dark:bg-gray-900 rounded-[2rem] p-6 shadow-sm border border-gray-100 dark:border-gray-800 transition-all">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="text-[17px] font-bold text-gray-900 dark:text-white tracking-tight">{section.title}</h2>
            </div>
            {section.description && <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">{section.description}</p>}
            {section.content}
          </div>
        ))}

        <div className="pt-12 pb-8 text-center px-8">
          <p className="text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-[0.2em] leading-relaxed opacity-60">
