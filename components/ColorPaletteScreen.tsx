...
export const ColorPaletteScreen: React.FC<ColorPaletteScreenProps> = ({ onBack, onOpenCustom }) => {
  ...
  const isPreset = PRESET_ACCENTS.includes(accentColor);
  ...
  return (
    <div className="flex flex-col h-full bg-[#F8F9FA] dark:bg-[#0A0A0A] relative overflow-hidden transition-colors duration-300">
      <header className="px-6 pb-6 flex items-center justify-between sticky top-0 bg-[#F8F9FA]/80 dark:bg-[#0A0A0A]/80 backdrop-blur-md z-20" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 48px)' }}>
        ...
        <section className="mb-12">
          ...
          <div className="bg-white dark:bg-[#121212] rounded-[3rem] p-8 shadow-sm border border-gray-100 dark:border-gray-900 transition-all duration-500">
            <div className="grid grid-cols-5 gap-3 mb-10 mx-auto max-w-[300px]">
              ...
              >
                <Palette size={20} className={showCustomPicker || !isPreset ? (hsl.l > 75 ? 'text-black' : 'text-white') : 'text-gray-400'} />
              </button>
            </div>
