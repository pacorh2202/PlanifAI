...
export const ColorPaletteScreen: React.FC<ColorPaletteScreenProps> = ({ onBack, onOpenCustom }) => {
  ...
  const renderTemplateCard = (template: any, isSelected: boolean, onClick: () => void, onEdit?: () => void) => (
    <div 
      key={template.id}
      onClick={onClick}
      className={`bg-white dark:bg-[#121212] rounded-[2.5rem] p-7 relative border-2 transition-all duration-500 cursor-pointer shadow-sm ${
        isSelected ? 'ring-1 ring-opacity-20' : 'border-transparent'
      }`}
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <div 
            className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${
                isSelected ? 'shadow-lg' : 'bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700'
            }`}
                ...
                <div className="flex flex-wrap gap-4 mb-10 justify-center">
                    {PRESET_ACCENTS.map((color) => (
                        <button
                            ...
                            className={`w-12 h-12 rounded-full border-4 transition-all active:scale-90 ${
                                accentColor === color ? 'border-gray-900 dark:border-white scale-110 shadow-lg' : 'border-transparent'
                            }`}
                    ...
                    <button
                        ...
                        className={`w-12 h-12 rounded-full border-4 flex items-center justify-center transition-all active:scale-90 ${
                            showCustomPicker ? 'border-gray-900 dark:border-white scale-110 shadow-lg' : 'bg-gray-50 dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-400'
                        }`}
                        style={{ backgroundColor: showCustomPicker ? accentColor : '' }}
                    >
                        <Palette size={20} className={showCustomPicker ? 'text-white' : 'text-gray-400'} />
                    </button>
                </div>
