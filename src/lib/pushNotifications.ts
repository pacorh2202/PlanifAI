...
async function getDespiaSDK(): Promise<any> {
    if (_despiaSDK !== undefined) return _despiaSDK;
    try {
        const mod = await import('despia-native');
        _despiaSDK = mod.default ?? mod;
    } catch {
        _despiaSDK = null; // not in Despia environment
    }
    return _despiaSDK;
}
...
async function getOneSignalSubscriptionId(): Promise<string | null> {
    try {
        ...
        if (isDespiaNative()) {
            ...
            const despia = await getDespiaSDK();
            // despia-native package exposes onesignalplayerid directly
            const immediateId = despia?.onesignalplayerid || (window as any).__onesignal_player_id;
            if (immediateId) {
                console.log('[PUSH] Despia player ID (immediate):', immediateId);
                return immediateId;
            }
            // Poll — may not be available immediately at app start
            return new Promise((resolve) => {
                let attempts = 0;
                const poll = setInterval(async () => {
                    const sdk = await getDespiaSDK();
                    const pid = sdk?.onesignalplayerid || (window as any).__onesignal_player_id;
                    if (pid) {
                        console.log('[PUSH] Despia player ID (polled):', pid);
                        clearInterval(poll);
                        resolve(pid);
                    } else if (attempts >= 20) {
                        console.warn('[PUSH] Despia player ID not available after 10s — giving up');
                        clearInterval(poll);
                        resolve(null);
                    }
                    attempts++;
...
export function initPushNotifications() {
    ...
    if (isCapacitorNative()) {
        ...
        // ⚠️ Webviews cannot register Service Workers — Web SDK init would fail.
        // The native Despia runtime handles push delivery; we only need to bind external_id
        // via the bridge, which is done in registerPushToken().
        console.log("[PUSH] Despia detected — skipping Web SDK init (no service worker support in webview)");
        if (_webInitPromiseResolve) _webInitPromiseResolve(); // unblock registerPushToken
...
export async function registerPushToken(userId: string): Promise<void> {
    ...
    try {
        if (isCapacitorNative()) {
            ...
            // Protocol: setonesignalplayerid://?user_id=<id>
            // (NOT onesignal://login — that doesn't exist in Despia's bridge)
            console.log('[PUSH] Binding player_id to user via Despia bridge for:', userId);
            try {
                const sdk = await getDespiaSDK();
                if (sdk && typeof sdk === 'function') {
                    sdk(`setonesignalplayerid://?user_id=${encodeURIComponent(userId)}`);
                    console.log('[PUSH] ✅ Despia bridge (function): setonesignalplayerid called for', userId);
                }
                // Also set via window.despia (legacy write-based bridge)
                (window as any).despia = `setonesignalplayerid://?user_id=${encodeURIComponent(userId)}`;
                console.log('[PUSH] ✅ Despia bridge (window): setonesignalplayerid called for', userId);
            } catch (e) {
                console.error('[PUSH] Despia bridge setonesignalplayerid failed:', e);
            }

        } else if (isWebPush()) {
            // CRITICAL: Must wait for init() to finish before calling login()
            console.log('[PUSH] Waiting for Web SDK init to complete (Web)...');
